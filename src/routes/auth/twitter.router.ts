...
const getUser: (ctx: Context) => IUserModel = (ctx: Context) => ctx.request.query.user || ctx.state.user;
...
class TwitterRouter {
    ...
    static async migrateView(ctx: Context): Promise<void> {
        ...
        const user: IUserModel = getUser(ctx);
        if (!user) {
    ...
    static async migrate(ctx: Context): Promise<void> {
        ...
        const sessionUser: IUserModel = getUser(ctx);
        if (!sessionUser) {
            logger.info('No user found in current session when presenting the migration form. Redirecting to the migration start page.');
            return ctx.redirect('/auth/twitter/start');
        }
        ...
        const user: IUserModel = await UserService.getUserById(sessionUser.id);
        if (!user) {
            error = 'Could not find a valid user account for the current session';
        }

        const migratedUser: IUserModel = await UserService.migrateToUsernameAndPassword(user, ctx.request.body.email, ctx.request.body.password);
