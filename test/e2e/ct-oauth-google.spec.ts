...
describe('Google auth endpoint tests', () => {
    ...
    it('Visiting /auth/google/callback while being logged in should redirect to the login successful page', async () => {
        ...
        const confirmedUser: IUserDocument = await UserModel.findOne({ email: 'john.doe@vizzuality.com' })
            .exec();
    ...
    it('Visiting /auth/google/token with a valid Google OAuth token should generate a new token', async () => {
        ...
        const existingUser: IUserDocument = await UserModel.findOne({ email: 'john.doe@vizzuality.com' })
            .exec();
        ...
        const userWithToken: IUserDocument = await UserModel.findOne({ email: 'john.doe@vizzuality.com' })
            .exec();
    ...
    it('Visiting /auth/google/token with a valid Google OAuth token should generate a new token - account with no email address', async () => {
        const savedUser: IUserDocument = await new UserModel({
            name: 'John Doe',
            role: 'USER',
            provider: 'google',
            providerId: '113994825016233013735',
        ...
        const existingUser: IUserDocument = await UserModel.findOne({ _id: savedUser.id })
            .exec();
        should.exist(existingUser);
        existingUser.should.have.property('name')
            .and
        ...
        const userWithToken: IUserDocument = await UserModel.findOne({ _id: savedUser.id })
            .exec();
        should.exist(userWithToken);
        userWithToken.should.have.property('name')
            .and
